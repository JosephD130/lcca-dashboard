...
table(s, [
  ...
  ['TDOT mark', 'Row 1 is a 40pt band on all thirteen sheets a user can reach, with the mark at the right at the artwork aspect. The four placements that existed were each drawn 8 to 11 percent too tall. Row 1 is now a print title, so the mark prints on every page; the worked example also went from 41 printed pages to 30 once the alternative worksheets stopped printing their chart-data columns'],
  ...
  ['Pay item picker', 'The drop-down split its width evenly between the item number and the description, so most descriptions read as fragments. All 105 combo boxes now carry an explicit 9 inch list width and the picker column is wider, so the number and the whole description both read'],
  ['Salvage', 'The asphalt salvage fraction was a fixed 12.5 percent against a table that places the overlay at year 20, where the arithmetic is 37.5 percent; the concrete 25 percent was right at a 30-year period and no other. Both are now computed from remaining life, with the expected life shown in its own column. Murfreesboro asphalt moves from $8,809,266 to $8,572,110, concrete unchanged, margin $780,532 to $543,376'],
  ['Priced by division', 'The Middle, West and East columns on Pay_Items had never been read. Every alternative worksheet now picks one in C11, defaulting to Regular, and prices from it; a blank regional cell falls back to the statewide unit cost, so the columns can be filled item by item'],
